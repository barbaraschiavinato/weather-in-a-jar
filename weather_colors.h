...
// ============================================================
//
// This file contains the visual appearance of the Weather Jar:
// - base colors
// - brightness per weather scenario
// - weather -> color mapping
// - weather -> solar visibility mapping
// - sunrise / sunset palettes
//
// Change this file to tune the look of the jar without changing
...
constexpr RgbColor COLOR_MAINLY_CLEAR = {
  245, 205, 150
};

constexpr RgbColor COLOR_PARTLY_CLOUDY = {
  220, 220, 205
};
...
constexpr RgbColor COLOR_DRIZZLE = {
  70, 125, 180
};
...
// Softer colors used when the sun is only partially visible.
constexpr RgbColor COLOR_PARTIAL_DEEP = {
  120, 35, 15
};

constexpr RgbColor COLOR_PARTIAL_ORANGE = {
  210, 95, 40
};

constexpr RgbColor COLOR_PARTIAL_WARM = {
  230, 175, 120
};
...
inline RgbColor getWeatherColor(
  WeatherType weather
) {
  switch (weather) {
    ...
    case WeatherType::MAINLY_CLEAR:
      return COLOR_MAINLY_CLEAR;

    case WeatherType::PARTLY_CLOUDY:
      return COLOR_PARTLY_CLOUDY;
...
// ============================================================
// WEATHER -> BRIGHTNESS MAPPING
// ============================================================

inline uint8_t getWeatherBrightness(
  WeatherType weather
) {
  switch (weather) {
    case WeatherType::CLEAR:
...
// ============================================================
// WEATHER -> SUN VISIBILITY
// ============================================================

inline SunVisibility getSunVisibility(
  WeatherType weather
) {
  switch (weather) {
    case WeatherType::CLEAR:
...
// ============================================================
//
// The current weather color is intentionally not part of these
// constant arrays because it is dynamic and is appended/prepended
// by the application when calculating the transition.
// ============================================================
...
constexpr int SUNRISE_FULL_COLOR_COUNT =
  sizeof(SUNRISE_FULL_COLORS) /
  sizeof(SUNRISE_FULL_COLORS[0]);

constexpr RgbColor SUNRISE_PARTIAL_COLORS[] = {
  COLOR_OFF,
  COLOR_PARTIAL_DEEP,
  COLOR_PARTIAL_ORANGE,
  COLOR_PARTIAL_WARM
};

constexpr int SUNRISE_PARTIAL_COLOR_COUNT =
  sizeof(SUNRISE_PARTIAL_COLORS) /
  sizeof(SUNRISE_PARTIAL_COLORS[0]);
...
constexpr int SUNSET_FULL_COLOR_COUNT =
  sizeof(SUNSET_FULL_COLORS) /
  sizeof(SUNSET_FULL_COLORS[0]);
...
constexpr int SUNSET_PARTIAL_COLOR_COUNT =
  sizeof(SUNSET_PARTIAL_COLORS) /
  sizeof(SUNSET_PARTIAL_COLORS[0]);
